...
fn execute_claude_mcp_command(args: Vec<&str>) -> Result<String> {
    info!("Executing claude mcp command with args: {:?}", args);

    let claude_path = crate::claude_binary::find_claude_binary().map_err(|e| anyhow::anyhow!(e))?;
...
pub async fn mcp_add(
    ...
    match execute_claude_mcp_command(cmd_args) {
        Ok(output) => {
...
pub async fn mcp_list() -> Result<Vec<MCPServer>, String> {
    ...
    match execute_claude_mcp_command(vec!["list"]) {
        Ok(output) => {
...
pub async fn mcp_get(name: String) -> Result<MCPServer, String> {
    info!("Getting MCP server details for: {}", name);

    match execute_claude_mcp_command(vec!["get", &name]) {
        Ok(output) => {
...
pub async fn mcp_remove(name: String) -> Result<String, String> {
    ...
    match execute_claude_mcp_command(vec!["remove", &name]) {
        Ok(output) => {
...
pub async fn mcp_add_from_claude_desktop(
    ...
    for (name, server_config) in mcp_servers {
        ...
        match mcp_add_json(name.clone(), json_str, scope.clone()).await {
            Ok(result) => {
                if result.success {
                    imported_count += 1;
                    server_results.push(ImportServerResult {
...
pub async fn mcp_serve() -> Result<String, String> {
...
pub async fn mcp_reset_project_choices() -> Result<String, String> {
    info!("Resetting MCP project choices");

    match execute_claude_mcp_command(vec!["reset-project-choices"]) {
        Ok(output) => {
