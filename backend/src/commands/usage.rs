...
// No pricing table: Claude Code writes the authoritative cost into the JSONL
// stream itself (`costUSD` on assistant messages, `total_cost_usd` on result
// messages). Historical entries without either field contribute zero — the
// template does not attempt to reconstruct cost from tokens + a rate card, as
// those rate cards drift and fork users would inherit stale numbers.
...
struct JsonlEntry {
    ...
    cost_usd: Option<f64>,
    /// `result`-type lines carry the authoritative rollup cost here.
    #[serde(default)]
