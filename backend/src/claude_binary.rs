...
///
/// Honors the `APP_CLAUDE_BINARY` env var when set (explicit override), then
/// falls back to system discovery. The previous Tauri-specific DB-backed
/// preference path was removed with the desktop app.
pub fn find_claude_binary() -> Result<String, String> {
    info!("Searching for claude binary...");

    if let Ok(explicit) = std::env::var("APP_CLAUDE_BINARY") {
        let p = PathBuf::from(&explicit);
        if p.exists() && p.is_file() {
            info!("Using APP_CLAUDE_BINARY override: {}", explicit);
            return Ok(explicit);
        }
        warn!(
            "APP_CLAUDE_BINARY is set to {} but that path does not exist",
            explicit
        );
    }
